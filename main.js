 ...
 */
function test1() {
  const umlObjectDiagram = app.repository.select("@UMLObjectDiagram")
  ...
  let oUMLDiagrams = [];
  umlObjectDiagram.forEach(element => {
    if (element instanceof type.UMLObjectDiagram) {
      oUMLDiagrams.push(element);
    }
  });
  console.log(oUMLDiagrams)
  vDialog = null;
  let dm = app.dialogs;
  vDialog = dm.showModalDialog("", "toto", "toto2", [], true);
  setTimeout(function () {
    starTestingAllDiagram(oUMLDiagrams);
  }, 10);
}
...
function init() {
  ...
  /* Register command to Generate Specification */
  app.commands.register('atcg01:test', test1);
